...
# Advisory: worklog conflict check reminder
if echo "$COMMAND" | grep -qE 'git\s+commit\b'; then
    WORKLOG="${CLAUDE_PROJECT_DIR:-.}/.claude/worklog.md"
    if [ -f "$WORKLOG" ]; then
        ACTIVE_COUNT=$(sed -n '/@ACTIVE/,/@LOG/{//d;/^#/d;/^$/d;p}' "$WORKLOG" | wc -l)
        if [ "$ACTIVE_COUNT" -gt 1 ]; then
            echo "WORKLOG_ADVISORY: ${ACTIVE_COUNT} active sessions. Verify no conflicts before committing."
        fi
    fi
fi
